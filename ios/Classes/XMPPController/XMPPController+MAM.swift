...
extension XMPPController {
    func getMAMMessage(
        ...
        if !isEmptyJid {
            ...
            let aJIDField = XMPPMessageArchiveManagement.field(
                withVar: "with",
                type: nil,
                andValue: jidString)
        ...
        printLog(
            "\(#function) | req mam: since \(tsSince) | JIDString: \(jidString) | Limit \(defaultLimit)")
    ...
    func manageMAMMessage(message: XMPPMessage) {
        ...
        let vMessType: String = (message.mamResult?.forwardedMessage?.type ?? xmppChatType.NORMAL).trim()
        switch vMessType {
        case xmppChatType.CHAT, xmppChatType.GROUPCHAT:
            self.handel_MAMMessage(message, withType: vMessType, withStrem: self.xmppStream)
    ...
    func xmppMessageArchiveManagement(
        _ xmppMessageArchiveManagement: XMPPMessageArchiveManagement, didReceiveFormFields iq: XMPPIQ
    ) {
    ...
    func xmppMessageArchiveManagement(
        ...
        didFailToReceiveFormFields iq: XMPPIQ
    ) {
        printLog("\(#function) | xmppMAM | iq: \(String(describing: iq))")
    }
    ...
    func xmppMessageArchiveManagement(
        _ xmppMessageArchiveManagement: XMPPMessageArchiveManagement,
        didFailToReceiveMessages error: XMPPIQ?
    ) {
    ...
    func xmppMessageArchiveManagement(
        ...
        guard let mamResult = message.mamResult else {
