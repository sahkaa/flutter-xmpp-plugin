...
extension XMPPController: XMPPRoomDelegate {
    func createRoom(withRooms arrRooms: [groupInfo], withStrem: XMPPStream) {
        for objRoom in arrRooms {
            ...
            guard
                let roomJID = XMPPJID(
                    string: getXMPPRoomJidName(withRoomName: roomName, withStrem: withStrem))
            else {
    ...
    func joinRoom(roomName: String, time: Int64, withStrem: XMPPStream) {
        ...
        // xmppRoom.fetchConfigurationForm()
        printLog(
            "\(#function) | perform activity of Join XMPPRoom | \(roomName) | userId: \(vUserId) | history: \(history)"
        )
    }
    ...
    func updateGroupInfoIntoXMPPRoomCreatedAndJoined(
        withXMPPRoomObj roomXMPP: XMPPRoom, roomName vRoom: String
    ) {
    ...
    func xmppRoom(_ sender: XMPPRoom, didFetchConfigurationForm configForm: DDXMLElement) {
        ...
        for field in arrRoomConfig {
            ...
            case "muc#roomconfig_persistentroom":
                ...
                printLog(
                    "\(#function) | XMPPRoom Configuration | \(roomProparty) | defaultConfig: \(defaultConfig)"
                )
                ...
                if let objRoomInfo = self.arrGroups.first(where: { (obj) -> Bool in
                    ...
                    printLog(
                        "\(#function) | XMPPRoom Configuration | \(roomProparty) | update-Config: \(field)")
                }
    ...
    func xmppStream(_ sender: XMPPStream, didReceive iq: XMPPIQ) -> Bool {
        ...
        if let eleError = iq.childErrorElement {
            ...
            if let valueErrorMess = eleError.getElements(withKey: "text").first?.stringValue {
                vErrorMess = valueErrorMess.trim()
            }
            let isErrorIQ =
                (vCode == "403") && (vErrorMess.lowercased() == xmppConstants.errorMessOfMUC.lowercased())
            printLog(
                "\(#function) | isErrorIQ: \(isErrorIQ) | vCode: \(vCode) | vErrorMess: \(vErrorMess)")
...
extension XMPPController {
    ...
    func getRoomMember(
        withUserType vType: xmppMUCUserType,
        ...
        guard
            let index = self.arrGroups.firstIndex(where: { (objGroup) -> Bool in
        ...
        printLog(
            "\(#function) | perform activity of get XMPPRoom Member | room: \(roomName) | role: \(vType)")
        switch vType {
        case .Member:
    ...
    func addRemoveMemberInRoom(
        withUserRole vRole: xmppMUCUserType,
        ...
        printLog(
            "\(#function) | perform activity of XMPPRoom Member - \(actionType) | room: \(roomName) | role: \(vRole)"
        )
    ...
    func getAllMemeberInfo(withItems items: [Any], withUserRole vRole: xmppMUCUserType) {
        ...
        for objUser in items {
            ...
            var vJid: String = ""
            if let value = eleUser.attributeStringValue(forName: "affiliation") {
                vAffiliation = value.trim()
            }
            if let value = eleUser.attributeStringValue(forName: "jid") { vJid = value.trim() }
            printLog(
                "\(#function) | eleUser: \(eleUser) | eleUser-affiliation: \(vAffiliation) | eleUser-jid: \(vJid) | role: \(vRole)"
            )
    ...
    func xmppRoom(_ sender: XMPPRoom, didFetchMembersList items: [Any]) {
        printLog(
            "\(#function) | Get XMPPRoom Members | sender: \(sender) | items-count: \(items.count)")
